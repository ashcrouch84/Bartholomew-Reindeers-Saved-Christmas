...
<html>
	...
	<body>
		...
		<p><center><font size = "6" color="white">Toy Factory Information</font></center></p>
	...
	<center><form action="Toy.php" method=post enctype='multipart/form-data'></center>
			...
			if($response['valid']=="")
				...
				else
					{
						if(isset($_REQUEST['check']))
							{
								$PCheck = ($_REQUEST['check']);
								$ID = ($_REQUEST['Qid']);
								$PRef = $response['bookings'][$ID]['orderId'];
								$PName = $response['bookings'][$ID]['name'];
								$RemoteFolder = "Saved";

								if ($PCheck == 2)
									{
										$ToSave = $PRef.",".$PName;
										$file = $PRef."_Saved.txt";
										$filename = $RemoteFolder."/".$file;

										file_put_contents($filename,$ToSave);
										echo "<br>";
										$x= $PName." has saved Christmas and been uploaded successfully";
										echo "<p align='left'><b><font color=white font face='arial' size='4'>".$x."</font></b></p>";
									}
							}
				...
				if($response['valid']<>"1")
					...
					{		
						...
						{
							?>
								<option value=<?php echo $i; ?>><?php echo $response['bookings'][$i]['name']; ?></option>	
							<?php
							$i=$i+1;
						}
						?>
							<center><font size = "8">Name: </font></center>
							<center><font size = "12"><input type="text" name="QName" value=""></font></center>
							<input type="hidden" name="check" value="2">
							<input type="hidden" name="QDate" value=<?php echo $PDate; ?>>
							<input type="hidden" name="QTime" value=<?php echo $PTime; ?>>
							</select>
							<input type=submit value='Saved Christmas' style="font-size:25px"> 
							<br>
							...
							while ($i < $count+1)
								...
								while ($i < $count)
									...
									if ($hiddenX[$familyCount-1] == "Hidden")
										{
									?>
										<form action="Toy.php" method=post enctype='multipart/form-data'>
												<a>
													<span></span>
													<span></span>
													<span></span>
